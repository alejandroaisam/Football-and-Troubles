...
The following code corresponds to the section II of the master's thesis. More specifically, it is devoted to the construction of various generalized linear models to analyse the effect of football matches in protests. As an aclanatory note, the interpretations are present in the proper thesis. 
...
For this section of the thesis we load the database concerning the matches of Cliftonville. To allow the regression work, some variables are recoded as factor. We also include the lagged variables for the negative binomial models. 

Similarly, matches are usually played on Sundays. Thus, we establish that the actual variable to predict is the number of protests of the next week. Similarly, some week-district had two matches. However, they cannot be removed as they entail different events, which can affect very differently to the proper outcome. 
...
summary(log_com); msme::P__disp(log_NI)
...
The negative binomial model is trained with all the previous variables + the lagged variables. Additionally, we use the *glm.nb* funtion form the `MASS` package. 
...
Similarly, we can finally run the zero inflated negative binomila model with the *zeroinfl* function from `pscl`package. 
...
To consider the over-dispersion of data, a quasi-poisson model may be trained:
