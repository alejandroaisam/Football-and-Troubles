...
This notebook will contain all the steps in a single file. The main purpose is to unify the process so the replication code for the master's thesis is easier and more understandable. Small text will be added to each chunk of code to explain the logic of the step and/or clarify some issues. 

## 1. Treating the raw data. 

The first step did in the proccess of writing the thesis correspondent to the extraction and management of the data needed for the analysis: the data on protests and the data for the football matches. 
...
In order to get the data, I downloaded the files in [Ron Francisco's webpage](https://ronfran.ku.edu/data/index.html) refered to both Northern Ireland and Republic of Ireland. At the begining of the proccess the Republic of Ireland was to be included in the analysis. It was ultimately removed, but its data was already treated. At this step, I will only prepare the data for the analysis; make the column names more understandable and add time and location. 

#### Northern Ireland 

First step is to include all the data in *~/Databases/Protests/Original*. From them, we will unify all the files into single dataframes referring to a region and store them in *~/Databases/Protests/* with the name `NI_Raw`. 
...
Once the data is unified, I renamed some date columns, and create others, and add the district for the different population. Originally, As a small warning, the following part was done in a different script. Moreover, it is possible that some locations are wrongly distributed. This step was done by hand, using the 26 districts that used to divide the region between 1972 and 2015 as reference. Also, the issues non-related to *The Troubles* are defined at this stage.  
...
NIProtestsDF <- NI_Raw %>% 
  mutate(
    fecha = Date, # Probably not necessary now. 
    Date = dmy(Date)) %>% 
  rename(
    ...
    N_StateForces = `# State`) %>% 
  mutate( 
    District = case_when( # Separate Script!!!
...
Once we have the data cleaned and more understandable, I merged both territories and filtered out the protests that were not related to the *The Troubles*
...
ProtestsFiltered <- ProtestsDF %>% 
  ...
  filter(!(Issue %in% IssuesRemoval)) # Separate Script!!
...
The problem with the matches of Derry City lies on the fact that I had not had access to a unified dataframe. Thus, I had to first extract them from [Extratime.com](https://www.extratime.com/teams/results/7/derry-city/?results=1987). For that, I created the following custom function: 
...
This function allow me to extract the name of the competition, the date of the match, the opponent, the ground and the result. Instead of doing a loop over all the years of competition, I decided to apply the function individually for identification purposes.
...
Once we have the data for all the competition, I formatted the colums so I could later perform the cleaning operations. Then, I unified all the years into a single dataframe contanining all the matches. The resulting dataframe is the equivalent to the protests one before the cleaning (NI_Raw)
...
Then, I performed the same tasks as with the protests. I renamed some columns, and added the time and location. With respect to the location, a binary variable is created referring to the match played in Derry. Also, the dummy accounting for a Derry win is created. These two are the future independent variables. 
...
With respect to the religious data, I collected it from [Cain](https://cain.ulster.ac.uk/ni/popul.htm#cath). Then, as usual, I renamed the variables and create a dummy for those districts that possessed majority of catholics. While it would not be later used, it may had served in the analysis. 
...
Finally, I downloaded the GIS data for the 26 districts from [GADM](https://gadm.org/data.html) in shapefile format and stored them in *~/Data*. As the data incorporates all the United Kingdom, I needed to keep only the region of Northern Ireland. Then, I stored the new shapefile that accounts only for NI. 

```{r}
UK <- readOGR(dsn = "data", layer = "gadm36_GBR_3")
...
#writeOGR(obj = NI, dsn = "data", layer = "NI", driver = "ESRI Shapefile")

```

Then, I can calculte the distances to Derry and Belfast from all of the other districts. It has to be noted here that each district is composed by a polygon, and the distances are calculated from the centroids of each one.

Getting the coordinated and centroids:
...
Now we have all the data ready to be merged and aggregated into a single dataframe. This step is covered in the rmd known as 'Agregating.rmd'. 