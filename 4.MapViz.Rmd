...
As usual, we may load all the data stored in the repository. As stated in the previous step some specific featurings will be performed at each step. In this case, the number of protests per month, per year and per periods are to be calculated. 
...
Final_Derry <- read_csv("Databases/Final_Derry.csv") %>% 
  dplyr::select(-...1) %>% 
...
data1980 <- data_year |> 
  ...
  mutate(Period = "FirstPeriod")

data1984 <- data_year |> 
  filter(year > 1983 & year <= 1987) |> 
  group_by(District) |> 
  summarise(n_protests = sum(yearly_protests)) |> 
  mutate(Period = "SecondPeriod")

data1988 <- data_year |> 
  filter(year > 1987 &year <= 1991) |> 
  group_by(District) |> 
  summarise(n_protests = sum(yearly_protests)) |> 
  mutate(Period = "ThirdPeriod")

data1991 <- data_year |> 
  filter(year > 1991) |> 
  group_by(District) |> 
  summarise(n_protests = sum(yearly_protests)) |> 
  mutate(Period = "FourthPeriod")
...
The second map is the animated version of the first representation. A separate markdown can be found for knitting purposes. 
...
Third map: geographical represention of the log number of protests in four periods: 
